...
const CallCenter = () => {
    ...
    const fetchUsers = async () => {
        try {
            if (isChild) {
                // For children, we need to get ONLY the staff assigned to this child
                console.log("Fetching assigned staff for child ID:", myId);
                
                // First get the child's details to find their assigned teacher
                const childResponse = await axios.get(`${BASE_URL}/children/${myId}`);
                // The child data is nested inside a 'child' object
                const childData = childResponse.data.child; 
                console.log("Child data:", childData);
                
                // Extract the assigned_teacher_id from the nested child data
                const assignedTeacherId = childData.assigned_teacher_id;
                
                if (assignedTeacherId) {
                    // Now get the teacher's details using the assigned_teacher_id
                    const teacherResponse = await axios.get(`${BASE_URL}/teachers/${assignedTeacherId}`);
                    ...
                    const transformedTeacher = {
                        ...
                        name: teacherData.name || `${teacherData.first_name} ${teacherData.last_name}`,
                        initials: teacherData.first_name && teacherData.last_name 
                    ...
                    setUserList([transformedTeacher]);
                } else {
                    console.log("No assigned teacher found for this child.");
                    setUserList([]); // Set empty list if no teacher is assigned
                }
            } else {
                // For staff members, get all children and filter by assigned teacher
                console.log("Fetching children for staff ID:", myId);
                ...
                // Filter children to only show those assigned to this staff
                const filteredChildren = children.filter(child => 
                    parseInt(child.teacher_id) === parseInt(myId)
                );
                
                console.log("Filtered children:", filteredChildren);
                
                // Transform child data to match expected format
                const transformedChildren = filteredChildren.map(child => ({
                    user_id: child.user_id,
                    first_name: child.first_name || '',
                    last_name: child.last_name || '',
                    name: child.child_full_name || `${child.first_name} ${child.last_name}`,
                    initials: child.first_name && child.last_name 
                        ? `${child.first_name[0]}${child.last_name[0]}` 
                        : child.child_full_name 
                            ? child.child_full_name.split(' ').map(n => n[0]).join('')
                            : 'C'
                }));
                
                setUserList(transformedChildren);
            }
                ...
                <div className="col-md-4 border-end p-3">
                    ...
                    <ul className="list-group list-group-flush">
                        {userList.length > 0 ? (
                            ...
                            <li className="list-group-item text-muted">
                                {isChild 
                                    ? "No assigned staff found. Please contact an administrator." 
